...
def plot(x, y1, y2, title, y1_name='Primary Y-Axis', y2_name='Secondary Y-Axis', y1_color='blue', y2_color='red'):
    ...
    y1_range = [min(y1)-60, max(y1)+60]   # Set the range for the primary y-axis to cover both y1 and y2
    y2_range = [min(y1)-60, max(y1)+60]  # Set the range for the secondary y-axis to cover both y1 and y2
